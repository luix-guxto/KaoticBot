//Bot baseado na IRIS de KillovSky, para ver os creditos, basta entrar me KillovSky.txt, ou ver a LICENSE
...
const { owner } = require('./lib/lang/pt')
...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
		...
		case 'gps':
			await kill.sendText(from, mess.gps())
			break
    ...
    //       break
		...
		/*case 'legiao':
			...
			case 'gp': // by luix
				...
				kill.reply(from, `Qual o grupo deseja o link?\n\nCaso tenha duvida digite ${prefix}gps.`, id)
			}
			switch (grupoMsgCliente){
			case '1':
				kill.getGroupInviteLink('557588737769-1620501506@g.us').then((a) => kill.sendLinkWithAutoPreview(from, a, `\n\nGrupo oficial do Kaotic Bot`))
				break
			case '2':
				kill.getGroupInviteLink('557588737769-1559309169@g.us').then((a) => kill.sendLinkWithAutoPreview(from, a))
				break
			case '3':
				kill.getGroupInviteLink('553199949012-1601559160@g.us').then((a) => kill.sendLinkWithAutoPreview(from, a))
				break
			case '4':
				kill.getGroupInviteLink('557588737769-1553711859@g.us').then((a) => kill.sendLinkWithAutoPreview(from, a))
				break
			default:
				kill.reply(from, `Qual o grupo deseja o link?\n\nCaso tenha duvida digite ${prefix}gps.`, id)
		}
       ...
       case 'clima':
        ...
        case 'kick':;case 'k':;case 'ban':;case 'k':;case 'b':
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
				if (!isBotGroupAdmins) return await kill.reply(from, mess.botademira(), id)
				if (quotedMsg) {
		...
		case 'allid':;case 'grupos'://espera ai
		...
		case 'gruposl': // by luix
			if(!(isOwner)) return await kill.reply(from, mess.sodono, id) // fiz isso por causa do bug, pode fuder o grupo
			if(isGroupMsg) return await kill.reply(from, `Piranha só no pv que eu mando nud`, id)
			const todosOsGrupos = await kill.getAllGroups()
			for(let ids of todosOsGrupos){
			kill.getGroupInviteLink(''+ids.contact.id).then((a) => kill.sendLinkWithAutoPreview(from, a, `\n => ${ids.contact.name}`))
			}			
			break
		...
		case 'help':
			...
			await kill.sendText(config.help2, mess.advise(nopvne, isgorp, user, body, hpgp))
			await kill.sendText(config.help1, mess.advise(nopvne, isgorp, user, body, hpgp))
		...
		case 'cpf':
			...
			try {
				...
				await kill.reply(from, mess.wait(), id)
				const browsercf = await puppeteer.launch({ headless: false, defaultViewport: null })
		...
		case 'steal':;case 'lula':;case 'dilma':
		...
		case 'jokenpo':
			...
			break
				...
				if (!isOwner) return await kill.reply(from, mess.sodono(), id)
				   if(quotedMsg) return await kill.reply(from, `Não marque ninguem!`, id)
				var usuarioAleatorio = await randomMember
				var usuarioText = usuarioAleatorio.replace(/@c.us/g, '') 
		
				await kill.sendTextWithMentions(
					from, 
					`═✪〘 🎸 - 🐂 〙✪═ \n\n @${usuarioText}\n\n═✪〘 👉 ${args[0]} 〙✪═`
				)
				
				var usuarioXp = await kill.getContact(usuarioAleatorio)
				var usuarioGanhaXp = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? usuarioXp.id : user)
				var xpProUsuario = parseInt(args[0])
				if (isNaN(xpProUsuario)) return await kill.reply(from, mess.onlynumber(), id)
				await sleep(2000)
				await addXp(usuarioGanhaXp, Number(xpProUsuario), nivel)
				await kill.sendTextWithMentions(from, mess.gainxp(usuarioAleatorio, xpProUsuario) + 'XP.')
				
				break */ //essa dá a xp pra quem digita o comando

				case 'sorteio': //by luix

				if (!isOwner) return await kill.reply(from, mess.sodono(), id)
				if(!isGroupMsg) return await kill.reply(from, `Por que está fazendo sorteio no privado?`, id)
				   if(quotedMsg) return await kill.reply(from, `Não marque ninguem!`, id)
				   try{
					   var experiencia = parseInt(args[0])
				   }catch(e){
					   return kill.reply(from, mess.onlynumber, id)
				   }
				   if(isNaN(experiencia)) return kill.reply(from, mess.onlynumber, id)
				...
				var xpProUsuario = experiencia
				await sleep(2000)
				await addXp(usuarioAleatorio, Number(xpProUsuario), nivel)
				break

				case 'verdade':
					const verdade = await fs.readFileSync('./lib/config/Utilidades/verdade.txt').toString().split('\n')
					
								 const ve = verdade[Math.floor(Math.random() * verdade.length)]
									await kill.sendFile(from, './lib/media/img/verdade.jpg', 'vdd.jpg', ve, id)
									
									break
									case 'desafio':
										const desafio = await fs.readFileSync('./lib/config/Utilidades/desafio.txt').toString().split('\n')
			const de = desafio[Math.floor(Math.random() * desafio.length)]
			await kill.sendFile(from, './lib/media/img/desafio.jpg', 'dsf.jpg', de, id)
									break //esboço para game de verdade ou desafio

									case 'testando':
										number=args[0]
										kill.sendContact(from, `${number}@c.us` |`${number}@c.us`["teste"])
									break
        ...
        default:
			...
			await kill.sendText(config.help1, mess.wpprpt(command, err))
			await kill.sendText(config.help2, mess.wpprpt(command, err))
		await kill.reply(from, mess.fail(), id)
