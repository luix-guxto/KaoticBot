...
//	if (kaoticvs.version !== getversion.data.version) { console.log(color('\n[UPDATE]', 'crimson'), color(`Uma nova versão da Íris foi lançada [${getversion.data.version}], atualize para obter melhorias e correções! → ${kaoticvs.homepage}`, 'gold')) }
	console.log(color('\n[SUPORTE]', 'magenta'), color(`55 31 99994-9012 | +55 75 98873-7769 | ${kaoticvs.bugs.url}\n`, 'lime'), color(`\n[KAOTIC BOT ${kaoticvs.version}]`, 'magenta'), color('Estamos prontos para começar mestre!\n', 'lime'))
		...
		try {
			if (event.action == 'add') {
				if (isAnti && fuck && !isMyBot) {
					...
					.setBackground('https://i.pinimg.com/originals/7c/f9/6e/7cf96e5878cc8aad3e704b81150d4d47.png')
					.toAttachment()
